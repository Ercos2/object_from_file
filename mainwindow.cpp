    ...
    if (File.open(QIODevice::ReadOnly)) {
        ...
        while (!stream.atEnd()) {
            Object_from_file temp_object;
                //initialize new objects using strings.
...
void MainWindow::on_add_button_clicked() {
    ...
    // checking the filling to add a new element
    if (s_name->size() < 1) {
    ...
    //we find out the time in MSecs and divide by 1000 to get Secs with a remainder in MSecs
    auto time = std::make_unique<QString>(QString(QString::number((QDateTime::currentMSecsSinceEpoch()/1000.0), 'f', 3)));
    ...
    for(int a = 0; a < file_objects->size(); ++a) {
        //we enter the current result in the line
        *output += (*file_objects)[a].get_output();
    }
    //output a string in TextEdit
    ui->textEdit->setText(*output);
    auto cursor = std::make_unique<QTextCursor>(QTextCursor(ui->textEdit->textCursor()));
    //move the cursor down to see the newly added element.
